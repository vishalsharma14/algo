"""
    ...
    file paths of the files that have the same content. A file path is a string that has the following format:
...
def dulpicate_files(files):
    """
        :param files: List of files with file content
        :return: List of list of duplicate file names
    """
    files_dict = {}
    output = []
    for file in files:
        ...
        for i in range(1, len(file_split)):
            file_name_content = file_split[i].split("(")
            file_name, file_content = file_name_content[0], file_name_content[-1]
            file_path = "{}/{}".format(root, file_name)
            if file_content not in files_dict:
                files_dict[file_content] = [file_path]
            else:
                files_dict[file_content].append(file_path)
    for key in files_dict:
        if len(files_dict[key]) > 1:
            output.append(files_dict[key])
    return output